...
export default class ShardManager extends EventEmitter {
  ...
  async validateAndCleanSession(sessionDirectory: string): Promise<void> {
    try {
      ...
      // Hanya bersihkan jika JSON corrupt, atau jika sudah registered tapi tidak valid
      if (status.exists) {
        const shouldClean = status.reason === "Corrupt JSON" || (status.registered && !status.valid);
        if (shouldClean) {
  ...
  private setupShardEventHandlers(sock: any, id: string, saveCreds: any, options: ShardOptions) {
    ...
    sock.ev.on("connection.update", async (update: any) => {
      ...
      if (connection === "open") {
        // Jika ada timer reconnect, hentikan karena sudah open
      ...
      if (connection === "close") {
        ...
        // Hindari multiple reconnect: satu timer per shard
        if (!this.#reconnectTimers.has(id)) {
          const timer = setTimeout(async () => {
            try {
              await this.recreateShard({ id, ...options, clearSession: false });
            } finally {
              this.#reconnectTimers.delete(id);
            }
          }, 5000);
  ...
  async createShard(options: ShardOptions = {}): Promise<{ id: string; sock: any }> {
    try {
      ...
      if (state.creds?.registered === true) {
        ...
        logger.warn(`Auth state shows not registered for ${id}, proceeding without clearing session`);
      }
